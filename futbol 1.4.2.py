...
ARENA_RADIUS = 260
BALL_RADIUS = 40
# Kale genişliğini biraz artırdım çünkü araya direkler girecek
GOAL_WIDTH_RADIANS = 0.28
POST_RADIUS = 8  # Direklerin kalınlığı/yarıçapı
GOAL_DEPTH = 60  # Kalenin derinliği
...
FRICTION = 0.99999
ELASTICITY = 0.95
POST_ELASTICITY = 1.1 # Direkler biraz daha sert sektirsin
...
pygame.display.set_caption("Futbol Simulasyonu - Direkten Donen Toplar")
...
def draw_real_goal(surface, cx, cy, radius, angle, width_rad):
    ...
    # 1. Fileyi (Zemini) Çiz (Yarı saydam)
    goal_poly = [(p1x, p1y), (b1x, b1y), (b2x, b2y), (p2x, p2y)]
...
class Ball:
    ...
    def collide_post(self, px, py):
        """Direklere carpma mantigi (Static Circle Collision)"""
        dx = self.x - px
        dy = self.y - py
        dist = math.hypot(dx, dy)

        # Eger top direge degiyorsa (Top yaricapi + Direk yaricapi)
        if dist < self.radius + POST_RADIUS:
            # Normal vektoru (Direkten topa dogru)
            nx, ny = dx / dist, dy / dist

            # Hızın normal yönündeki bileşeni (Dot Product)
            dot = self.vx * nx + self.vy * ny

            # Sadece top direge dogru gidiyorsa sektir
            if dot < 0:
                # Yansıma (Reflection)
                self.vx = (self.vx - 2 * dot * nx) * POST_ELASTICITY
                self.vy = (self.vy - 2 * dot * ny) * POST_ELASTICITY

                # Topu diregin icinden cikar (Overlap fix)
...
def start_match():
    global ball1, ball2, crowd, team1_name, team2_name, team1_colors, team2_colors, state, score1, score2, goal_events_1, goal_events_2, frame_counter, goal_rotating
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if state == "MENU":
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                start_btn_rect = pygame.Rect(WIDTH//2 - 100, HEIGHT - 100, 200, 60)
    ...
    if state == "MENU":
        ...
        screen.blit(btn_txt, (WIDTH//2 - btn_txt.get_width()//2, HEIGHT - 90))
        ...
        if state in ["FIRST_HALF", "SECOND_HALF"]:
            ...
            for b in [ball1, ball2]:
                ...
                # Sadece top direklerin arasindan gecerse gol sayilsin
                # Artik angle logic + distance yeterli degil, direk fizigi var
